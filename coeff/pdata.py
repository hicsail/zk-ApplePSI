...
def make_Cuckoo(secrets, p, Points, alpha, epsilon):
    ...
    lagrange_coeffs = lagrange_poly(xs, ys, p)
    
    # Ensure that coeffs actually work
    # check = calc_polynomial(0, lagrange_coeffs, p)
    # obj = ys[0]
    # print("\ncheck", check)
    # print("\nagainst", obj)
    # assert check.x == obj.x and check.y == obj.y
    
    for bot_idx in emptyList:
        bot = calc_polynomial(bot_idx, lagrange_coeffs, p)
    ...
    return cuckoo_table, non_emplist, lagrange_coeffs
